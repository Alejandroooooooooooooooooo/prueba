...
class UsersAPI(http.Controller):
    ...
    @http.route('/PrivacyPolicy_TermsOfService', auth='public', type='http')
    def politicas_de_privacidad(self):
        ruta_html = os.path.join(os.path.dirname(__file__), '../static/src/html/PrivacyPolicy_TermsOfService.html')
        try:
            with open(ruta_html, 'r', encoding='utf-8') as file:
                contenido = file.read()
            return Response(contenido, content_type='text/html')
